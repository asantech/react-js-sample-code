...
const getAuthDataMock = () => {
  ...
  const accessTokenMock = encryptTextWithSecretKey({
    text: `${currentTimeStamp}-${ACCESS_TOKEN_EXPIRE_DURATION}`,
    secretKey: SECRET_KEY_MOCK,
  })
  const refreshTokenMock = encryptTextWithSecretKey({
    text: `${currentTimeStamp}-${REFRESH_TOKEN_EXPIRE_DURATION}`,
    secretKey: SECRET_KEY_MOCK,
  })
  const authData = {
    auth: {
...
const setTokensToCookie = ({ accessToken, refreshToken }: AuthTokens) => {
  setCookie({
    name: "accessToken",
    value: accessToken,
    days: (1 / 24) * ACCESS_TOKEN_EXPIRE_DURATION,
  })
  setCookie({
    name: "refreshAccessToken",
    value: refreshToken,
    days: (3 / 24) * REFRESH_TOKEN_EXPIRE_DURATION,
  })
}

const requestNewAccessToken = ({ authTokens }: { authTokens: AuthTokens }) => {
  const currentTimeStamp = Date.now()
  const newAccessTokenMock = encryptTextWithSecretKey({
    text: `${currentTimeStamp}-${ACCESS_TOKEN_EXPIRE_DURATION}`,
    secretKey: SECRET_KEY_MOCK,
  })
...
export const makeAuthenticatedReq = async ({
  requestBody,
  requestLogic,
}: any) => {
...
export const signInMock = async (
...
): Promise<SignInMockResponse | undefined | unknown> => {
  ...
  const authResponseMock: Promise<unknown> = getValueAfterDelay(
    authDataMock,
    MOCK_RESPONSE_DELAY
  )
